-- FUNCTION: gwolofs.congestion_network_segment_agg(date, text, bigint[])

-- DROP FUNCTION IF EXISTS gwolofs.congestion_network_segment_agg(date, text, bigint[]);
...
DELETE FROM gwolofs.congestion_raw_segments
WHERE
    segment_id = ANY(congestion_network_segment_agg.segment_ids)
    AND dt = congestion_network_segment_agg.start_date;
...
ALTER FUNCTION gwolofs.congestion_network_segment_agg(date, text, bigint[])
    OWNER TO gwolofs;

GRANT EXECUTE ON FUNCTION gwolofs.congestion_network_segment_agg(date, text, bigint[]) TO PUBLIC;

GRANT EXECUTE ON FUNCTION gwolofs.congestion_network_segment_agg(date, text, bigint[]) TO congestion_bot;

GRANT EXECUTE ON FUNCTION gwolofs.congestion_network_segment_agg(date, text, bigint[]) TO gwolofs;

COMMENT ON FUNCTION gwolofs.congestion_network_segment_agg(date, text, bigint[])
