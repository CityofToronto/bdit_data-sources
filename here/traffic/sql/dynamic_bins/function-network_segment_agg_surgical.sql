/*
create table gwolofs.missing_segments AS 
select a.segment_id
from congestion.congestion_segments_24_4 a
left join here_agg.raw_segments b using (segment_id)
where b.segment_id is null
*/

-- FUNCTION: here_agg.network_segment_agg_surgical(date, text, bigint[])

-- DROP FUNCTION IF EXISTS here_agg.network_segment_agg_surgical(date, text, bigint[]);

CREATE OR REPLACE FUNCTION here_agg.network_segment_agg_surgical(
    start_date date,
    congestion_network_table text,
    segment_ids bigint[]
)
    RETURNS void
    LANGUAGE 'plpgsql'
...
DELETE FROM here_agg.raw_segments
WHERE
    segment_id = ANY(network_segment_agg_surgical.segment_ids)
    AND dt = network_segment_agg_surgical.start_date;
...
EXECUTE FORMAT(
    $$
        ...
        WITH segment_length_cte AS (
            SELECT
                ...
                link_dir,
                length,
                SUM(length) OVER (PARTITION BY segment_id) AS segment_length
            FROM congestion.%1$I
            WHERE segment_id = ANY(%3$L)
        )
    ...
    $$, congestion_network_table, start_date, segment_ids);
...
ALTER FUNCTION here_agg.network_segment_agg_surgical(date, text, bigint[])
OWNER TO here_admins;

GRANT EXECUTE ON FUNCTION here_agg.network_segment_agg_surgical(date, text, bigint[]) TO PUBLIC;

GRANT EXECUTE ON FUNCTION here_agg.network_segment_agg_surgical(date, text, bigint[]) TO congestion_bot;

COMMENT ON FUNCTION here_agg.network_segment_agg_surgical(date, text, bigint[])
...
/*
--example
SELECT here_agg.network_segment_agg_surgical('2025-01-01'::date, 'congestion_links_24_4'::text, (SELECT array_agg(segment_id) FROM gwolofs.missing_segments)::bigint[])
*/
