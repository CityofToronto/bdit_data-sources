-- FUNCTION: gwolofs.congestion_network_period_segment_agg(date)

-- DROP FUNCTION IF EXISTS gwolofs.congestion_network_period_segment_agg(date);

CREATE OR REPLACE FUNCTION gwolofs.congestion_network_period_segment_agg(
    start_date date)
    RETURNS void
    LANGUAGE 'plpgsql'
    COST 100
...
EXECUTE FORMAT(
    ...
    WITH time_bins AS (
        SELECT
            %1$L::date + start_tod AS start_time, --start_date
            %1$L::date + end_tod AS end_time, --start_date
            timerange(start_tod, end_tod, '[)') AS time_grp
        FROM
        (VALUES
    ...
    unnested_db_options AS (
        ...
        --makes sense to relax this constraint for periods
...
ALTER FUNCTION gwolofs.congestion_network_period_segment_agg(date)
    OWNER TO gwolofs;

COMMENT ON FUNCTION gwolofs.congestion_network_period_segment_agg(date)
    IS '''Dynamic bin aggregation of the congestion network by periods: 
            (''00:00:00''::time, ''06:00:00''::time),
            (''06:00:00''::time, ''10:00:00''::time),
            (''10:00:00''::time, ''15:00:00''::time),
            (''15:00:00''::time, ''19:00:00''::time),
