--DROP FUNCTION here_agg.corridor_bootstrap;

CREATE OR REPLACE FUNCTION here_agg.corridor_bootstrap(
    start_date date,
    end_date date,
    corridor_id bigint,
    n_resamples int,
...
AS $BODY$
    ...
    SELECT setseed(('0.'||corridor_id::text)::numeric);

    WITH raw_obs AS (
        SELECT
            --corridor_id and mnth don't need to be in group by until end
        ...
        WHERE -- same params as the above aggregation
            ...
            AND dt < corridor_bootstrap.end_date
            AND rs.hr >= corridor_bootstrap.hr_starts
            AND rs.hr < corridor_bootstrap.hr_ends
            AND corridor_id = corridor_bootstrap.corridor_id
            AND EXTRACT('isodow' FROM dt) = ANY(corridor_bootstrap.isodows)
            AND NOT (
                corridor_bootstrap.include_holidays = False
                AND EXISTS (
    ...
    random_selections AS (
        ...
        CROSS JOIN generate_series(1, corridor_bootstrap.n_resamples) AS sample_group(group_id)
        GROUP BY
            raw_obs.avg_tt,
            raw_obs.q1_tt,
            raw_obs.q2_tt,
    ...
    SELECT
        corridor_bootstrap.corridor_id,
        corridor_bootstrap.start_date,
        corridor_bootstrap.end_date,
        corridor_bootstrap.hr_starts,
        corridor_bootstrap.hr_ends,
    ...
    JOIN here_agg.corridors AS cs
        ON cs.corridor_id = corridor_bootstrap.corridor_id
    GROUP BY
        hr_starts,
        hr_ends,
        avg_tt,
...
/*Usage example: (works best one corridor at a time with Lateral)
--16s 8 corridors, 1 week
SELECT wkdy_grps.*, lat.*
FROM UNNEST('{2,3,4,5,6,7,8,9}'::bigint[]) AS unnested(corridor_id),
(VALUES
...
LATERAL (
    SELECT * FROM here_agg.corridor_bootstrap(
                    ...
                    corridor_id := corridor_id::bigint,
                    n_resamples := 300::int,
