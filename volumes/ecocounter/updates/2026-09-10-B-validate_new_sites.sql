...
SELECT DISTINCT site_id, count_date from temp_ecocounter_changes
WHERE change = 'anomalous_range' 
	AND site_id NOT IN (
    SELECT site_id
   FROM ecocounter.anomalous_ranges
   WHERE upper(time_range) IS NULL); -- did not realise there was only one until I made this table
...
SELECT uid, flow_id, site_id, time_range, notes, investigation_level, problem_level
