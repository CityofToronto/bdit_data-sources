WITH out_of_order AS (
    SELECT
        ...
        || ' (' || '{{ macros.ds_add(ds, 7) }}'::date - MAX(c.datetime_bin::date) -- noqa: TMP, LT05
        || ' days)`' AS description
    ...
    WHERE
        c.datetime_bin >= '{{ macros.ds_add(ds, 7) }}'::date -- noqa: TMP
        - interval '{{ params.lookback }}' -- noqa: TMP
        AND c.datetime_bin < '{{ macros.ds_add(ds, 7) }}'::date + interval '1 day' -- noqa: TMP, LT05
    GROUP BY
    ...
    HAVING
        ...
        < '{{ macros.ds_add(ds, 7) }}'::date - interval '{{ params.min_duration }}' -- noqa: TMP, LT05
),
