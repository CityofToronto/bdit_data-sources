...
class MiovPuller:
    ...
    def get_road_class(self, row, conn):
        """Get road user class."""
    ...
    def process_tmc_row(self, row, conn):
        """Process one row of TMC API output."""

        classification = self.get_road_class(row, conn)
    ...
    def process_response(self, apitype, response, conn):
        ...
        return [(self.intersection_uid, )
                + self.process_tmc_row(row, conn) for row in data]

    def get_intersection(self, start_time, end_iteration_time, conn):
        ...
        table_veh = self.process_response('tmc', response_tmc, conn)
        response_crosswalk = self.get_response(
            'crosswalk', start_time, end_iteration_time)
        table_ped = self.process_response('crosswalk', response_crosswalk, conn)
