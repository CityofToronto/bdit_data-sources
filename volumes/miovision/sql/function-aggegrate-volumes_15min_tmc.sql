...
BEGIN
	...
	FROM (SELECT intersection_uid, datetime_bin, SUM(volume) AS total_volume FROM miovision.volumes WHERE volume_15min_tmc_uid IS NULL GROUP BY intersection_uid, datetime_bin) B
	INNER JOIN (SELECT intersection_uid, datetime_bin, SUM(volume) AS total_volume FROM miovision.volumes WHERE volume_15min_tmc_uid IS NULL GROUP BY intersection_uid, datetime_bin) C ON B.intersection_uid = C.intersection_uid
