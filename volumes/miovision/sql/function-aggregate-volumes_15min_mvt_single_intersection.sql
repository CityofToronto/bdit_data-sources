...
WITH aggregate_insert AS (
	...
	LEFT JOIN miovision_api.unacceptable_gaps AS un ON
        un.intersection_uid = i.intersection_uid
	...
	LEFT JOIN miovision_api.volumes AS v ON
		...
		AND v.intersection_uid = i.intersection_uid
	...
	WHERE
        i.intersection_uid = intersection
        -- Only include dates during which intersection is active 
        -- (excludes entire day it was added/removed)
        AND dt.datetime_bin::date > i.date_installed + interval '1 day'
        AND (
            i.date_decommissioned IS NULL
            OR (dt.datetime_bin::date < i.date_decommissioned - interval '1 day')
        )
	...
	GROUP BY
        ...
        un.accept
	HAVING
        im.classification_uid IN (1,2,6,10) --retain 0s for certain modes (padding)
        OR SUM(v.volume) > 0 
