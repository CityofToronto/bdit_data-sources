WITH ongoing_outages AS (
    SELECT
        ...
        || MAX(v.datetime_bin::date) || ' (' || '{{ macros.ds_add(ds, 7) }}'::date --noqa: TMP, LT05
        - MAX(v.datetime_bin::date) || ' days)' AS descrip
    FROM miovision_api.volumes AS v
    JOIN miovision_api.intersections AS i USING (intersection_uid)
    WHERE
        v.datetime_bin >= '{{ macros.ds_add(ds, 7) }}'::date - interval '{{ params.lookback }}' --noqa: TMP, LT05
        AND v.datetime_bin < '{{ macros.ds_add(ds, 7) }}'::date + interval '1 day' --noqa: TMP, LT05
    GROUP BY
        i.intersection_uid,
        i.intersection_name
    HAVING
        MAX(v.datetime_bin::date)
        < '{{ macros.ds_add(ds, 7) }}'::date - interval '{{ params.min_duration }}' --noqa: TMP
)
