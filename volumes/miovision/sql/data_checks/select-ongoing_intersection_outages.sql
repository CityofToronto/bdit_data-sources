WITH ongoing_outages AS (
    SELECT
        i.intersection_name || ' (uid: ' || i.intersection_uid || ') - data last received: '
            || MAX(v.datetime_bin::date) || ' (' || '{{ data_interval_end }}'::date - MAX(v.datetime_bin::date) || ' days)' AS description
    FROM miovision_api.volumes AS v
    JOIN miovision_api.intersections AS i USING (intersection_uid)
    WHERE
        v.datetime_bin >= '{{ data_interval_end }}'::date - interval '{{ params.lookback }}'
        AND v.datetime_bin < '{{ data_interval_end }}'::date + interval '1 day'
    GROUP BY
        i.intersection_uid,
        i.intersection_name
    HAVING MAX(v.datetime_bin::date) < '{{ data_interval_end }}'::date - interval '{{ params.min_duration }}'
)
