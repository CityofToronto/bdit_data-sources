WITH ongoing_outages AS (
    SELECT
        ...
        || MAX(v.datetime_bin::date) || ' (' || '{{ data_interval_end }}'::date --noqa: TMP
        - MAX(v.datetime_bin::date) || ' days)' AS descrip
    FROM miovision_api.volumes AS v
    JOIN miovision_api.intersections AS i USING (intersection_uid)
    WHERE
        v.datetime_bin >= '{{ data_interval_end }}'::date - interval '{{ params.lookback }}' --noqa: TMP
        AND v.datetime_bin < '{{ data_interval_end }}'::date + interval '1 day' --noqa: TMP
    GROUP BY
        i.intersection_uid,
        i.intersection_name
    HAVING MAX(v.datetime_bin::date)
        < '{{ data_interval_end }}'::date - interval '{{ params.min_duration }}' --noqa: TMP
)
