...
CREATE OR REPLACE VIEW gwolofs.miovision_open_data_wide_15min AS (
    ...
    SELECT
        v.intersection_uid,
        v.datetime_bin,
        COALESCE(SUM(v.volume) FILTER (
            WHERE
                v.movement_uid = 3
                AND v.classification_uid = ANY(ARRAY[1]::int [])
                AND v.leg = 'S'
        ), 0) AS sb_car_r,
        COALESCE(SUM(v.volume) FILTER (
            WHERE
        ...
        COALESCE(SUM(v.volume) FILTER (
            WHERE
                v.movement_uid = 3
                AND v.classification_uid = ANY(ARRAY[4, 5, 9]::int [])
                AND v.leg = 'S'
        ), 0) AS sb_truck_r,
        COALESCE(SUM(v.volume) FILTER (
            WHERE
        ...
        COALESCE(SUM(v.volume) FILTER (
            WHERE
                v.movement_uid = 3
                AND v.classification_uid = ANY(ARRAY[3]::int [])
                AND v.leg = 'S'
        ), 0) AS sb_bus_r,
        COALESCE(SUM(v.volume) FILTER (
            WHERE
