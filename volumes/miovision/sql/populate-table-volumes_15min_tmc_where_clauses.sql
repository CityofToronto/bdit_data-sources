...
WHERE A.datetime_bin>'2018-06-01'
...
FROM (SELECT intersection_uid, datetime_bin, SUM(volume) AS total_volume FROM miovision.volumes WHERE datetime_bin>'2018-06-01' GROUP BY intersection_uid, datetime_bin) B
INNER JOIN (SELECT intersection_uid, datetime_bin, SUM(volume) AS total_volume FROM miovision.volumes WHERE datetime_bin>'2018-06-01' GROUP BY intersection_uid, datetime_bin) C ON B.intersection_uid = C.intersection_uid
