CREATE OR REPLACE FUNCTION miovision_csv.aggregate_15_min_tmc_june2020(
	start_date date,
	end_date date)
    RETURNS void
    LANGUAGE 'plpgsql'
...
WITH zero_padding_movements AS (
		...
		WHERE intersection_uid IN (SELECT intersection_uid FROM miovision_api.intersections 
				WHERE intersection_uid IN (33, 34, 35, 36, 37, 38, 39, 40) AND start_date::date >= date_installed 
...
ALTER FUNCTION miovision_csv.aggregate_15_min_tmc_june2020(date, date)
    OWNER TO czhu;

GRANT EXECUTE ON FUNCTION miovision_csv.aggregate_15_min_tmc_june2020(date, date) TO PUBLIC;

GRANT EXECUTE ON FUNCTION miovision_csv.aggregate_15_min_tmc_june2020(date, date) TO miovision_api_bot;

GRANT EXECUTE ON FUNCTION miovision_csv.aggregate_15_min_tmc_june2020(date, date) TO czhu;
