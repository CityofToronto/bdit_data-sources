-- FUNCTION: rdumas.aggregate_15_min_tmc()

-- DROP FUNCTION rdumas.aggregate_15_min_tmc();

CREATE OR REPLACE FUNCTION rdumas.aggregate_15_min_tmc_zeroes(
	)
    RETURNS void
    LANGUAGE 'plpgsql'
...
BEGIN
	RAISE NOTICE '% Function Start. Inserting into bins', timeofday();
...
-- IF one of two 1-minute time bins BEFORE and AFTER 15-minute bin are populated, assume no interpolation needed
	...
	SELECT 	B.intersection_uid,
		B.datetime_bin,
		COALESCE(A.classification_uid, C.classification_uid) classification_uid,
		COALESCE(C.leg, A.leg) leg,
        COALESCE(C.movement_uid, A.movement_uid) movement_uid,
		COALESCE(CASE WHEN B.interpolated = TRUE THEN SUM(A.volume)*15.0/(span*1.0) ELSE SUM(A.volume) END, 0) AS volume
	FROM bins B
	INNER JOIN (SELECT * FROM miovision.intersection_movements WHERE classification_uid IN (1,2,6,7) /*Only cars,bikes,and peds*/)C USING (intersection_uid)
	LEFT OUTER JOIN volumes A ON A.volume_15min_tmc_uid IS NULL
								AND C.intersection_uid = A.intersection_uid 
			 					AND (C.classification_uid = A.classification_uid OR A.classification_uid IN (3,4,5))
			 					AND C.leg = A.leg 
			 					AND C.movement_uid = A.movement_uid 
			 					AND B.start_time <= A.datetime_bin AND B.end_time >= A.datetime_bin 
	GROUP BY B.intersection_uid, B.datetime_bin, 
			 COALESCE(A.classification_uid, C.classification_uid), COALESCE( A.leg, C.leg), COALESCE(A.movement_uid, C.movement_uid), interpolated, span
	...
	FROM volumes_15min_tmc a --aggregate_insert a
