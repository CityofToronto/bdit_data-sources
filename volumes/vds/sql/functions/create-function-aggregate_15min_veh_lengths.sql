...
BEGIN
    ...
    SELECT
        ...
        COUNT(*) AS count,
        SUM(COUNT(*)) OVER (PARTITION BY rv.division_id, rv.vds_id, datetime_bin(rv.dt, 15))
            AS total_count
...
COMMENT ON FUNCTION vds.aggregate_15min_veh_lengths IS 'Function to aggregate 
`vds.raw_vdsvehicledata` into table `vds.veh_length_15min` by 
detector / 15min bins / 1m length bins.';
