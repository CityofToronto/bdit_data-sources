...
AS
 WITH valid_rows AS (
         SELECT acc_safe_copy."ACCNB",
          ...
          WHERE acc_safe_copy."ACCDATE"::date >= '1985-01-01'::date AND acc_safe_copy."ACCDATE"::date <= 'now'::text::date
        ), events_dat AS (
         SELECT a_1."ACCNB"::bigint AS accnb,
            ...
            upper(btrim(i.description)) AS traffic_control_cond,
                ...
                CASE
                    WHEN "left"(a_1."ACCNB"::text, 1) = '0'::text AND length(a_1."ACCNB"::text) = 10 AND a_1."ACCDATE" >= '2020-01-01 00:00:00'::timestamp without time zone AND a_1."ACCDATE" <= '2020-12-31 00:00:00'::timestamp without time zone THEN 'TPS'::text
                    WHEN a_1."ACCNB"::bigint >= 1000000000 THEN 'TPS'::text
                    WHEN a_1."ACCNB"::bigint >= 100000000 THEN 'CRC'::text
                    ELSE NULL::text
                END AS data_source
           FROM valid_rows a_1
             LEFT JOIN collision_factors.accloc b_1 ON a_1."ACCLOC"::text = b_1.accloc
             LEFT JOIN collision_factors.loccoord c_1 ON a_1."LOCCOORD"::text = c_1.loccoord
             LEFT JOIN collision_factors.acclass d ON a_1."ACCLASS"::text = d.acclass
             LEFT JOIN collision_factors.impactype k ON a_1."IMPACTYPE"::text = k.impactype
             LEFT JOIN collision_factors.visible e ON a_1."VISIBLE"::text = e.visible
             LEFT JOIN collision_factors.light f ON a_1."LIGHT"::text = f.light
             LEFT JOIN collision_factors.rdsfcond g ON a_1."RDSFCOND"::text = g.rdsfcond
             LEFT JOIN collision_factors.traffictl h ON a_1."TRAFFICTL"::text = h.traffictl
             LEFT JOIN collision_factors.trafctlcond i ON a_1."TRAFCTLCOND"::text = i.trafctlcond
        ), events_desc AS (
         SELECT a_1."ACCNB"::bigint AS accnb,
            date_part('year'::text, a_1."ACCDATE"::date) AS accyear,
