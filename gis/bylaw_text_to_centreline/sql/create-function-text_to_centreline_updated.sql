...
--STEP 2
	...
	oid1_geom_translated GEOMETRY := ST_GeomFromText(text_arr_oid1[2], 2952);
	...
	text_arr_oid2 TEXT[] := (CASE WHEN btwn2_orig LIKE '%point%' AND (btwn2_check NOT LIKE '% of %' OR btwn2_check LIKE ('% of ' || TRIM(btwn1)))
				...
				ELSE jchew._get_intersection_geom_updated(highway2, btwn2, direction_btwn2::TEXT, metres_btwn2::FLOAT, int_id1)
				END);

	oid2_int INT := (text_arr_oid2[3])::INT;
	oid2_geom  GEOMETRY := ST_GeomFromText(text_arr_oid2[1], 2952);
	oid2_geom_translated GEOMETRY := ST_GeomFromText(text_arr_oid2[2], 2952);
	
	--route centrelines between two intersection geoms using pg routing
	--after_routing TEXT[] := (SELECT * FROM jchew.get_lines_btwn_interxn(oid1_int, oid2_int))

	line_geom geometry := (CASE WHEN oid1_geom IS NOT NULL AND oid2_geom IS NOT NULL
						   THEN (SELECT ST_Union(ST_LineMerge(geom)) FROM jchew.get_lines_btwn_interxn(oid1_int, oid2_int))
						   ELSE NULL END);
	/* To test
...
*/
	...
	--name of the centreline
	centreline_name varchar := (SELECT lf_name FROM jchew.get_lines_btwn_interxn(oid1_int, oid2_int) LIMIT 1);

	--name of id to reference back to centreline table
	geo_id numeric := (SELECT lf_name FROM jchew.get_lines_btwn_interxn(oid1_int, oid2_int));
...
raise notice 'btwn1: % btwn2: % btwn2_check: %  highway2: % metres_btwn1: %  metres_btwn2: % direction_btwn1: % direction_btwn2: %', btwn1, btwn2, btwn2_check, highway2, metres_btwn1, metres_btwn2, direction_btwn1, direction_btwn2;


RETURN QUERY (SELECT geo_id, centreline_name, con, notice, line_geom, oid1_geom, oid1_geom_translated, oid2_geom, oid2_geom_translated);
