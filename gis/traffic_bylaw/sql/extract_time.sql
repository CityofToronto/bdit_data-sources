select time_day as og_timeday, legend_id, 
				 ...
				 case when time_day similar to '\d*:\d\d__.m._to_\d*:\d\d__.m.'
				 	        then array(select(to_timestamp(unnest(split_ref(time_day::text)), 'HH:MI a.m.')::time without time zone))
				 	  when (regexp_split_to_array(time_day, ','))[1] similar to '\d*:\d\d__.m._to_\d*:\d\d__.m.' 
 							...
 							then array[
									array(select(to_timestamp(unnest(split_ref((regexp_split_to_array(time_day, ','))[1])), 'HH:MI a.m.')::time without time zone)), 
									array(select(to_timestamp(unnest(split_ref((regexp_split_to_array(time_day, ','))[2])), 'HH:MI a.m.')::time without time zone))]
					  when (regexp_split_to_array(time_day, ','))[1] similar to '\d*:\d\d__.m._to_\d*:\d\d__.m.' 
							and ((regexp_split_to_array(time_day, ','))[2] not similar to '%\d*:\d\d__.m._to_\d*:\d\d__.m.'
 							and (regexp_split_to_array(time_day, ','))[2] not similar to '%except%')
							then array(select(to_timestamp(unnest(split_ref((regexp_split_to_array(time_day, ','))[1])), 'HH:MI a.m.')::time without time zone))	
					  when time_day similar to '%next_following_day'
							then array[
									array[
									...
									array[
										...
										'23:59:59'::time without time zone]]
					  when time_day = 'Anytime' 
							then array['00:00:00'::time without time zone,'23:59:59'::time without time zone]
