CREATE OR REPLACE FUNCTION congestion_events.delete_old_itsc_issue_locations()
RETURNS TRIGGER AS $$
BEGIN
    ...
    WITH latest AS (
        ...
        FROM congestion_events.itsc_issue_locations
        GROUP BY
            divisionid,
            issueid
    )
  ...
  DELETE FROM congestion_events.itsc_issue_locations AS iil
  USING latest
  WHERE
