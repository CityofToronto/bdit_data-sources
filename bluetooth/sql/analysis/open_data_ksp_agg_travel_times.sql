...
CREATE OR REPLACE VIEW open_data.ksp_agg_travel_time AS 
 WITH valid_corridors AS (
         SELECT x.dt,
           ...
           FROM ( SELECT a.bt_id,
             ...
             LEFT JOIN king_pilot.bt_corridors_exceptions t ON t.corridor_id_old = y.corridor_id AND x.dt <@ t.excluded_datetime
         ...
         SELECT z.street_name AS street,
            ...
            sum(u.tt) / 60.0 AS tt
           FROM valid_corridors u
             JOIN king_pilot.bt_corridors z USING (corridor_id)
          GROUP BY z.corridor_id, z.street_name, z.direction, u.dt, u.day_type, u.category, u.period_name, u.period_id, z.segments, z.from_intersection, z.to_intersection
 ...
 SELECT to_char(date_trunc('month'::text, pilot.dt::timestamp with time zone), 'Mon ''YY'::text) AS month,
    pilot.street,
    pilot.direction,
  ...
  WHERE pilot.category = 'Pilot'::text AND pilot.dt < date_trunc('month'::text, 'now'::text::date::timestamp with time zone) - interval '1 month'
  GROUP BY (to_char(date_trunc('month'::text, pilot.dt::timestamp with time zone), 'Mon ''YY'::text)), (date_trunc('month'::text, pilot.dt::timestamp with time zone)), pilot.street, baseline.from_intersection, baseline.to_intersection, pilot.direction, pilot.day_type, ((pilot.period || ' '::text) || baseline.period_range), baseline.tt
