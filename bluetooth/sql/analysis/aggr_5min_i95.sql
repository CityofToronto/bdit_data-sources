...
CREATE MATERIALIZED VIEW bluetooth.aggr_5min_i95 AS 
 ...
 SELECT obs.analysis_id,
    ...
    count(1) AS obs
   FROM temp_aggr obs
  WHERE obs.measured_time::numeric > (obs.travel_time - 1.5 * obs.std_time) AND obs.measured_time::numeric < (obs.travel_time + 1.5 * obs.std_time)
