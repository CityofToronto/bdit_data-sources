...
- TPANA `geom` (and thus `length`) columns are known to be incorrect. May require work with the vendor/manual effort to clean up geometries.
- The start and end nodes should be accurate, although they may not be sufficiently accurate for routing: see [this script](./sql/wip_route_tpana.sql) for an attempt at routing between start and end nodes along the centreline.
<p align="left">
...
| geom                                         | geometry                    |  | Route geoms are calculated from `encoded_polyline` using a trigger when row is added to the database. Column is known to contain errors for division_id = 8026. |
| start_node                                   | geometry                    |  | Route start node calculated from `encoded_polyline`. |
| end_node                                     | geometry                    |  | Route end node calculated from `encoded_polyline`.   |
