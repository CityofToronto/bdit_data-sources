...
SELECT analysis_id, datetime_bin, tt, obs
FROM bluetooth.aggr_15min
WHERE analysis_id IN (116749,1387517,1388022,130557,1391996) AND datetime_bin IN (SELECT datetime_bin FROM gardiner_bins);

INSERT INTO bluetooth.aggr_15min_temp
SELECT analysis_id, datetime_bin, tt, obs
FROM bluetooth.aggr_15min
WHERE analysis_id IN (1393119,1396036,1396053) AND datetime_bin IN (SELECT datetime_bin FROM lakeshore_bins);
