...
def _validate_yyyymm_range(yyyymmrange):
    ...
    if len(yyyymmrange) != 2:
        raise ValueError('{yyyymmrange} should contain two YYYYMM arguments'.format(yyyymmrange=yyyymmrange))
    ...
    for yyyymm in yyyymmrange:
        if regex_yyyymm.fullmatch(yyyymm):
            ...
            raise ValueError('{yyyymm} is not a valid year-month value of format YYYYMM'.format(yyyymm=yyyymm))

    if yyyy[0] > yyyy[1] or (yyyy[0]==yyyy[1] and mm[0] > mm[1]):
        raise ValueError('Start date {yyyymm1} after end date {yyyymm2}'.format(yyyymm1=yyyymmrange[0],yyyymm2=yyyymmrange[1]))
...
def _validate_yearsjson(yearsjson):
    ...
    for year in yearsjson:
        if not regex_yyyy.fullmatch(str(year)):
            ...
            raise ValueError('For year {year}, month {mm} is not a valid month'.format(year=year, mm=yearsjson[year][0]))
        elif not regex_mm.fullmatch(str(yearsjson[year][1])):
            raise ValueError('For year {year}, month {mm} is not a valid month'.format(year=year, mm=yearsjson[year][1]))
        elif yearsjson[year][0] > yearsjson[year][1]:
            raise ValueError('For year {year}, first month {mm1} comes after second month {mm2}'.format(year=year, mm1=yearsjson[year][0], mm2=yearsjson[year][1]))
...
if __name__ == "__main__":
    ...
    YEARS_ARGUMENTS.add_argument("-y", "--years", nargs=2,
                                 help="Range of months (YYYYMM) to operate over from startdate to enddate",
                                 metavar=('YYYYMM', 'YYYYMM'))
    YEARS_ARGUMENTS.add_argument("-Y", "--yearsjson",type=json.load,              
                        help="Written dictionary which contains years as key and start/end month like {'2012'=[1,12]}")
